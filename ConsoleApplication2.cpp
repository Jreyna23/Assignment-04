// Dtrack Assignment 04.cpp : Defines the entry point for the console application.
//
...
int main()
{
	...
	cout << "Dtrack is online and ready to help find dogo!" << endl << endl;
	do // This forces the player to play at least once. This is where we will restart the loop.
	{
		...
		{
			cout << "Alright Owner, guess where the dogo is on an 8x8 grid (1-64)" << endl; // Prompt user to enter their guess
			...
			{
				humanTries++; // add 1 to tries, so we know how many attempts it took the player
				cout << "Dogo sighted at " << targetPos << endl; // print the block that we found our target at.
				cout << "Located " << humanTries << " trys to find the your Dogo." << endl << endl; // Print the number of tries it took us to get the correct number
			}
		...
		do // Do loop! This runs at least once! Binary Search
		{
			...
			cout << "Initiating Dtrack Binary search at position " << guess << "." << endl;
			...
			else
			{
				ai1Tries++; // add 1 to tries, so we know how many attempts it took the computer
				cout << "Dogo sighted at " << targetPos << endl; // print the block that we found our target at.
				cout << "Located  " << ai1Tries << " it took Binary Dtrack to find your dogo. " << endl << endl; // Print the number of tries it took us to get the correct number
		...
		for (int i = 0; i <= targetPos; i++) // Linear Search
		{
			...
			{
				cout << "Dogo not in cell " << i << "." << endl;
			}
			else // if the iteration of our search is equal to the targets position
			{
				cout << "Dogo sighted at " << i << "." << endl;
				cout << "It took " << ai2Tries << " Linaer Dtrack # of tries to find your Dogo." << endl;
			}
		...
		while (true) // Random Search
		{
			...
			{
				cout << "Dogo not found at " << randomAIPos << endl;
				randomAIPos = rand() % 64 + 1; // pick a new random number
			}
			else // if the random AI's guess is equl to the targets position
			{
				cout << "Dogo found at " << targetPos << endl;
				cout << "It took AI 3 " << ai3Tries << " tries to find the cell." << endl;
				break; // break the loop when we have found the targets position
			}

		}
		cout << "The binary AI took " << ai1Tries << " to find the Dogo." << endl; // Results 
		cout << "The linear AI took " << ai2Tries << " to find the Dogo." << endl; // Results
		cout << "The Random AI took " << ai3Tries << " to find the Dogo." << endl; // Results
		cout << "You took " << humanTries << " trys to find the Dogo." << endl; // Results 
